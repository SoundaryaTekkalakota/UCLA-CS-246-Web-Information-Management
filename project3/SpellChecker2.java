...
public class SpellChecker2 {
    ...
    public double calculateLM(String word) {
      ...
      Double totalFreq=0.00;
      for (Integer summ : dict.values()) {totalFreq=totalFreq+summ;}

      return dict.containsKey(word) ?  dict.get(word)/totalFreq  : 0.0;
    ...
    public double calculateEM(String word, String correction, double editDistance) {
        ...
        StringBuilder wordvowel = new StringBuilder();
        StringBuilder correctionvowel = new StringBuilder();

        // Smaller edit distance=>more weighted
        if(word.equals(correction)) {
            errorModel = Integer.MAX_VALUE;
        }
        ...
        // Since vowels are the most likely to be misused, more weight given if the strings are the same minus the vowels
        for(int i = 0; i < word.length(); ++i) {
            if(word.charAt(i) != 'a' && word.charAt(i) != 'e' &&
               word.charAt(i) != 'i' && word.charAt(i) != 'o' &&
               word.charAt(i) != 'u' && word.charAt(i) != 'y') {
                wordvowel.append(word.charAt(i));
            }
        }
        for(int j = 0; j < correction.length(); ++j) {
            if(correction.charAt(j) != 'a' && correction.charAt(j) != 'e' &&
               ...
               correction.charAt(j) != 'u' && correction.charAt(j) != 'y') {
                correctionvowel.append(correction.charAt(j));
            }
        }
        if(wordvowel.equals(correctionvowel)) {
            errorModel *= 70;
        }

        // Sound-alike letters b, e and c, s can also lead to errors
        for(int k = 0; k < word.length() && k < correction.length(); ++k) {
            if(word.charAt(k) == 's' && correction.charAt(k) == 'c' ||
               ...
               word.charAt(k) == 'b' && correction.charAt(k) == 'e' ||
               word.charAt(k) == 'e' && correction.charAt(k) == 'b' ||
